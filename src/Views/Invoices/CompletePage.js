...
export default function CompletePage() {
  ...
  useEffect(() => {
    ...
    const invoice_id = parseInt(new URLSearchParams(window.location.search).get("invoice_id"));
    ...
    stripe.retrievePaymentIntent(clientSecret).then(({ paymentIntent }) => {
      ...
      if (paymentIntent.status === "succeeded") {
        getInvoiceById(invoice_id).then((response) => {
          if (response.header.fen_solde_du <= 0) {
            return;
          }

          let newTransaction = {
            tra_date: moment().format("YYYY-MM-DD HH:mm:ss"),
            tra_value: paymentIntent.amount / 100,
            tra_ent_fk: response.header.fen_ent_fk,
            tra_desc: "Paiement de la facture par stripe : " + paymentIntent.payment_method,
            tra_fen_fk: invoice_id
          };

          TransactionService.addNewTransaction(newTransaction).then(() => {
            let invoiceData = { fen_id: response.header?.fen_id, fen_solde_du: 0, fen_etat: "1" };
