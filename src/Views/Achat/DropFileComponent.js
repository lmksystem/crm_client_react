...
function DropFileComponents({ values = [], setValues = () => {}, touched, errors, label = "" }) {
  function handleAcceptedFiles(files) {
    ...
    let newArray = copyValues?.files?.concat(files.map((item) => ({ ...item, idFile: genRandonString(15) })));
      ...
      {values?.files?.length > 0 && (
        <div
          ...
          {values?.files?.map((item, index) => (
            <div
              className="d-inline-flex d-flex justify-content-between align-items-center mt-2"
